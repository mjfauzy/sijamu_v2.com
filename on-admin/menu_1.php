...
<?php 
  ...
  if($jabatan == "Kepala Unit Jaminan Mutu") {
    $result_dokumen = mysqli_query($connect,"SELECT * FROM tbl_dokumen WHERE status NOT IN ('Tidak Berlaku','Telah Dihapus') AND kode_org='SBM.5' AND id_user NOT IN ('$id_user')");
  }
          ...
          <tr class="text-center">
            ...
            <td>
              ...
              <?php
                ...
                if($jabatan == 'Staf Unit Jaminan Mutu' && ($status == "Draf" || $status == "Telah Diperiksa")) {
            ...
            <td class="text-right">
              <?php 
                if($status == 'Draf') {
                  if($jabatan == 'Kepala Unit Jaminan Mutu') {
                    echo "Draf | <a href='#' class='open_modal_checked btn btn-success' id='".$row_dokumen->id."' title='Checked Document'><span class='glyphicon glyphicon-checked'></span> Checked</a> ";
                    echo "<a href='#' class='open_modal_tracking btn btn-primary' id='".$row_dokumen->id."' title='Show Document Tracking'><span class='glyphicon glyphicon-road'></span> Tracking</a>";
                  } else {
                    echo "Belum Diperiksa | <a href='#' class='open_modal_tracking btn btn-primary' id='".$row_dokumen->id."' title='Show Document Tracking'><span class='glyphicon glyphicon-road'></span> Tracking</a>";
                  }
                } else {
